...
class BaseModel(object):
    def __init__(self, hparams, iterator, scope=None):
        ...
        self.logit, self.final_result, self.attention_out = self._build_graph(hparams)###
        ...
        self.diversity_loss = self._compute_diversity_loss(hparams, self.final_result, self.attention_out)###add
        self.loss = tf.add(tf.add(self.data_loss, tf.negative(self.diversity_loss)), self.regular_loss)#########
    ...
    def _compute_diversity_loss(self, hparams, final_result, attention_out):##################add
        if hparams.diversity == 'NCL':
            diversity_loss = tf.zeros([1], dtype=tf.float32)
            diversity = []
            for i in range(0,len(final_result)):
                final_result_p = tf.reduce_sum(final_result[i], 2)
                ave = tf.reduce_mean(tf.reduce_sum(final_result[i], 2), 1)
                ave_expanded = tf.expand_dims(ave, 1)

                diversity.append(tf.reduce_mean(tf.square(final_result_p - ave_expanded), 1))
                
            diversity_temp_result = tf.reshape(diversity, shape=[-1, len(final_result), 1])
            
            diversity_loss = tf.reduce_sum(tf.multiply(attention_out, diversity_temp_result), 2)
            diversity_loss = tf.reduce_sum(diversity_loss, 1)
            diversity_loss = tf.reduce_mean(diversity_loss)

        elif hparams.diversity == 'NCL2':
            diversity_loss = tf.zeros([1], dtype=tf.float32)
            diversity = []
            for i in range(0,len(final_result)):
                final_result_x = final_result[i]
                ave = tf.reduce_mean(final_result_x, 1)
                ave_expanded = tf.expand_dims(ave, 1)
                
                temp_sub = final_result_x - ave_expanded
                diversity.append(tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square(temp_sub), 2)), 1))
            ...
            diversity_loss = tf.reduce_sum(diversity_loss, 1)
            diversity_loss = tf.reduce_mean(diversity_loss)
            #tf.reduce_mean(tf.multiply(attention_out, diversity_loss), 1)
            
        else:
            diversity_loss = tf.zeros([1], dtype=tf.float32)
            diversity_loss = tf.reduce_mean(diversity_loss)
        
        diversity_loss = tf.multiply(0.7, diversity_loss)
    ...
    def _add_summaries(self):
        ...
        tf.summary.scalar("diversity_loss", self.diversity_loss)###
    ...
    ####################################################add
    @abc.abstractmethod
    #def _build_diversity_loss(self, hparams):
    #    """Subclass must implement this."""
    #    pass
    ####################################################add
    ...
    def train(self, sess):
        return sess.run([self.update, self.loss, self.data_loss, self.diversity_loss, self.regular_loss, self.merged, self.final_result], \
                        feed_dict={self.layer_keeps: self.keep_prob_train})

    def eval(self, sess):
        return sess.run([self.loss, self.data_loss, self.diversity_loss, self.pred, self.iterator.labels], \
